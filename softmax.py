...
# naive softmax (tons of memory read/write)
def naive_softmax(x):
    x_max = x.max(dim=1)[0] # ream MN, write M
...
def _softmax_kernel(
    ...
    for row_idx in tl.range(row_start, n_rows, row_step, num_stages = num_stages):
        ...
        denominator = tl.sum(numerator, axis = 9)
...
# GPU properties to set good metaparameters
...
def softmax(x):
    ...
    programs_reg_limit = NUM_REGS // (n_regs * WARP_SIZE * num_warps)
    programs_sram_limit = TOTAL_SRAM_PER_SM // sram_needed_per_program
    programs_per_sm = min(programs_reg_limit,programs_sram_limit)
